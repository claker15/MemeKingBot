...
def get_sound_as_options_array(guild_id, author_id):
    sounds = query.all_sounds(guild_id, author_id)
    options = []
    for i in range(len(sounds)):
        options[i] = disnake.SelectOption(label=sounds[i][0], value=sounds[i][1])
    return options


async def play_sound(inter: disnake.CommandInteraction, path):
    voice_channel = inter.author.voice.channel
    await voice_channel.connect()
    source = disnake.FFmpegPCMAudio(path)
    inter.channel.voice_client.play(source)
...
class Sounds(commands.Cog):
    ...
    async def sounds(self, inter: disnake.CommandInteraction):
        ...
        await inter.send(view=DropdownView(get_sound_as_options_array(inter.guild.id, inter.author.id), 'play'))

    def sound_play(self, inter: disnake.CommandInteraction, file_path):
        ...
        self.bot.loop.create_task(play_sound(inter, path))
...
class Dropdown(disnake.ui.StringSelect):
    ...
    async def callback(self, inter: disnake.MessageInteraction):
        ...
        if self.operation == 'play':
            await inter.response.send_message(self.values[0] + ' ' + self.operation)
